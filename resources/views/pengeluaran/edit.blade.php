...
<div class="space-y-8">
    ...
    <div class="bg-white p-8 rounded-xl shadow-md border border-gray-200">
        ...
        <form action="{{ route('pengeluaran.update', $pengeluaran->id_pengeluaran) }}" method="POST" enctype="multipart/form-data" class="space-y-6" x-data="{ status: '{{ old('status_bayar', $pengeluaran->status_bayar) }}' }">
            @csrf
            @method('PUT')
            
            {{-- [FIX] TAMBAHKAN INPUT HIDDEN UNTUK WAKTU INPUT --}}
            <input type="hidden" name="waktu_input" value="{{ $pengeluaran->waktu_input }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="toko" id="toko" class="w-full p-2 border border-gray-300 rounded-lg" required>
                        @foreach($vendors as $vendor)
                            <option value="{{ $vendor }}" {{ old('toko', $pengeluaran->toko) == $vendor ? 'selected' : '' }}>
                                {{ $vendor }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="status_bayar" id="status_bayar" class="w-full p-2 border border-gray-300 rounded-lg" required x-model="status">
                        <option value="Sudah Bayar" {{ old('status_bayar', $pengeluaran->status_bayar) == 'Sudah Bayar' ? 'selected' : '' }}>Sudah Bayar / Lunas</option>
                        <option value="Belum Bayar" {{ old('status_bayar', $pengeluaran->status_bayar) == 'Belum Bayar' ? 'selected' : '' }}>Belum Bayar</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_struk" id="tanggal_struk" class="w-full p-2 border border-gray-300 rounded-lg" value="{{ old('tanggal_struk', $pengeluaran->tanggal_struk->format('Y-m-d')) }}" required>
                </div>
                ...
                <div x-show="status === 'Belum Bayar'">
                    ...
                    <input type="date" name="tanggal_bayar" id="tanggal_bayar" class="w-full p-2 border border-gray-300 rounded-lg" value="{{ old('tanggal_bayar', optional($pengeluaran->tanggal_bayar)->format('Y-m-d')) }}">
                </div>
                
                {{-- Rincian Item --}}
                <div class="md:col-span-2">
                    <label for="items" class="block text-sm font-medium text-gray-700 mb-1">Rincian Item</label>
                    <textarea name="items" id="items" rows="4" class="w-full p-2 border border-gray-300 rounded-lg" placeholder="Produk | Satuan | Harga&#10;Contoh: Besi | 2 kg | 500000" required>{{ old('items', $pengeluaran->items) }}</textarea>
                </div>
